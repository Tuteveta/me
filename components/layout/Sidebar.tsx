          ...
          {visible.map(item => {
            ...
            return (
              <li key={item.href}>
                ...
                >
                  <item.icon
                    className={`w-4 h-4 flex-shrink-0 ${active ? 'text-blue-700' : 'text-gray-400'}`}
                  />
          ...
          <div className={`px-3 py-2.5 flex items-center gap-2 ${collapsed ? 'justify-center' : ''}`}>
            ...
            {!collapsed && (
              <div className="min-w-0 flex-1">
                ...
                <span className={`text-[10px] font-bold capitalize px-1.5 py-0.5 rounded ${ROLE_COLORS[user.role]}`}>
                  {user.role}
                </span>
              </div>
            )}
